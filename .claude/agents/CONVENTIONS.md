...
- A "Resolve test grep tag" step computes the `--grep` argument the test-run step applies: `@smoke` if `test_subset=smoke` *or* the trigger is `schedule` (forced, see below — the scheduled run has no inputs to read), `@regression` if `test_subset=regression`, and no `--grep` at all for `test_subset=everything`, push, or pull_request. Push/PR behavior is unchanged from before this filtering existed — full suite, no filter.
