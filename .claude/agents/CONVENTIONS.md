...
- A "Resolve test grep tag" step computes a `grep_arg` output (the `--grep` argument the test-run step applies) and a `folder` output (the capitalized top-level report folder name, `Smoke` or `Regression`), using identical two-way branching for both: `@regression`/`Regression` if `test_subset=regression`, else (covers `test_subset=smoke`/default, `schedule`, and ordinary push/PR which have no inputs at all) `@smoke`/`Smoke`. Regression has no automatic trigger of its own — it only ever runs via a manual `workflow_dispatch` with `test_subset=regression`.
- Reports are nested run-folder-first, tool-second: `reports/Smoke/<timestamp>/allure/`, `reports/Smoke/<timestamp>/playwright/`, and the same under `reports/Regression/...` (the top-level folder name is the `grep-tag` step's `folder` output — literally capitalized `Smoke`/`Regression`, no `everything` bucket anymore). Each top-level folder maintains its own independent `latest` — `reports/Smoke/latest/{allure,playwright}/`, `reports/Regression/latest/{allure,playwright}/` — so a scheduled smoke run only ever updates `Smoke/latest` and never touches `Regression/latest`. The root `index.html` on `gh-pages` is generated as one section per folder (Smoke / Regression), each with its own "Latest Allure/Playwright Report" links plus its own historical timestamped links, built by looping over the two folder subdirectories. The "Output Allure/Playwright report URL" echo steps print the folder-qualified path with tool last (`reports/<Folder>/<timestamp>/<allure|playwright>/index.html`). `gh-pages` still has old runs under both the previous tool-first-by-subset layout (`reports/<allure|playwright>/<smoke|regression|everything>/...`) and the even older pre-subset flat layout (`reports/allure/<timestamp>/` directly) sitting alongside the new run-folder-first layout — the "Remove all except index.html and reports directory" step keeps the whole `reports/` tree regardless of layout, so old and new coexist without conflict; no migration was done or needed, and none is planned. There is also still no pruning/retention of old timestamped run folders — now hourly smoke runs add roughly 24 timestamped folders/day to `gh-pages` with nothing removing them; this was confirmed and explicitly left out of scope.
