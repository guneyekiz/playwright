...
- Also supports manual `workflow_dispatch` with two inputs: a `dev`/`qa` `environment` choice (default `dev`), and an `everything`/`smoke`/`regression` `test_subset` choice (default `everything`, matching the historical no-filter behavior). The job sets `environment: ${{ github.event.inputs.environment || 'dev' }}`, which resolves `BASE_URL`/`TEST_USERNAME`/`TEST_PASSWORD`/`BASIC_AUTH_USERNAME`/`BASIC_AUTH_PASSWORD` from that GitHub Environment's secrets. Both `dev` and `qa` Environments exist with their own copies of all five. A new env var needs its secret added to **both** Environments by a maintainer in the GitHub UI — no agent can create a GitHub secret, only document that one is needed and where.
- A "Resolve test grep tag" step computes both a `grep_arg` output (the `--grep` argument the test-run step applies) and a `subset` output (`smoke`/`regression`/`everything`), using identical branching for both: `@smoke`/`smoke` if `test_subset=smoke` *or* the trigger is `schedule` (forced, see below — the scheduled run has no inputs to read), `@regression`/`regression` if `test_subset=regression`, and no `--grep`/`subset=everything` for `test_subset=everything`, push, or pull_request. Push/PR behavior is unchanged from before this filtering existed — full suite, no filter.
- Reports are nested by subset: `reports/allure/<subset>/<timestamp>/` and `reports/playwright/<subset>/<timestamp>/` (where `<subset>` is `smoke`, `regression`, or `everything`, from the `grep-tag` step's `subset` output), not a flat `reports/allure/<timestamp>/`. Each subset maintains its own independent `latest` — `reports/allure/<subset>/latest`, `reports/playwright/<subset>/latest` — so a scheduled smoke run only ever updates `smoke/latest` and never touches `regression/latest` or `everything/latest`. The root `index.html` on `gh-pages` is generated as one section per subset (Smoke / Regression / Everything), each with its own "Latest Allure/Playwright Report" links plus its own historical timestamped links, built by looping over the three subset subdirectories rather than a single flat `ls`. The "Output Allure/Playwright report URL" echo steps print the subset-qualified path. `gh-pages` still has old runs under the pre-subset flat layout (`reports/allure/<timestamp>/` directly) sitting alongside the new nested layout — the "Remove all except index.html and reports directory" step keeps the whole `reports/` tree regardless of layout, so old and new coexist without conflict; no migration was done or needed.
- Scheduled smoke run: `schedule: - cron: '0 12 * * *'`, every day. GitHub Actions cron is UTC-only and `schedule` triggers cannot carry `workflow_dispatch` inputs, so the job detects `github.event_name == 'schedule'` directly and forces `@smoke` regardless of any `test_subset` value, and (via the existing `|| 'dev'` fallback) runs against the `dev` Environment since `github.event.inputs` is empty on a schedule trigger. **DST caveat, intentional, not a bug**: `0 12 * * *` is 07:00 EST / 08:00 EDT. US Eastern runs on EDT roughly 8 of the year's 12 months (mid-March–early November), so this cron hits the actual 8am target most of the year and drifts to 7am only across the ~4 winter months on EST. The cron expression carries a comment in the YAML explaining this tradeoff — don't "fix" the apparent hour drift without re-reading that comment first.
