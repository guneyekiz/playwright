...
- `playwright.config.ts`: testDir `./tests`, fullyParallel, retries/workers gated on `process.env.CI`, loads `dotenv/config` at the top, `use.baseURL` comes from `process.env.BASE_URL` (set in `.env`, untracked — see `.env.example` for the pattern), trace `on-first-retry`, reporters are `allure-playwright`, `html`, and `json`. Currently only the `chromium` project is configured — ask before adding firefox/webkit/mobile projects.
