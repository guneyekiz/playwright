...
- Name: example2.spec.ts >> has title
- Location: tests/example2.spec.ts:3:5
...
```ts
  ...
  5  | 
  6  |   // Expect a title "to contain" a substring.2222
> 7  |   await expect(page).toHaveTitle(/Playwright2/);
     |                      ^ Error: expect(page).toHaveTitle(expected) failed
  8  | });
  9  | 
